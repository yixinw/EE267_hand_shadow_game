...
public class displayHand : MonoBehaviour {
	...
	void Update () {
		...
		counter += 1;
		if (counter > 5) {
			// add 2 after Hello, World! to tell server which shadow to load
			s_tcp.writeSocket ("Hello, World!2");
...
public class s_TCP : MonoBehaviour {
	...
	public byte[] readSocket() {
		...
		if (theStream.DataAvailable) {
			...
			int numBytesRead;
			while (theStream.DataAvailable && (numBytesRead = theStream.Read (data, 0, data.Length)) > 0) {
				ms.Write (data, 0, numBytesRead);
			}
			// ret = Encoding.ASCII.GetString (ms.ToArray (), 0, (int)ms.Length);
			return ms.ToArray ();
